...
print("Loading Model...")
model = AutoModelForSequenceClassification.from_pretrained("meedan/SpanishPolitics", num_labels = 2)
print("Loading Tokenizer...")
tokenizer = AutoTokenizer.from_pretrained("meedan/SpanishPolitics")
...
async def is_politics(text: Text):
    ...
    if val_pred.predictions[0][0] > val_pred.predictions[0][1]:
        ...
        is_politics = 1
    to_return = {"is_politics": is_politics,"prob_is_not_politics": str(val_pred.predictions[0][0]),
                "prob_is_politics": str(val_pred.predictions[0][1]),"text": text.text}
    json_compatible_item_data = jsonable_encoder(to_return)
